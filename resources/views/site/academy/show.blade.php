...
<div class="col-md-2 col-12" style="padding-top:15px; margin-top: 145px!important;border-bottom-right-radius: 50px!important;border-bottom-left-radius: 50px!important;">  
...
<div class="col-4 col-md-4" style="font-size:13px">
<div class="row text-center">
<div class="col-12 col-md-12">{{ $package->time }}</div>
<div class="col-12 col-md-12"> ساعت</div>
</div>
</div>
<div class="col-4 col-md-4" style="font-size:13px">
<div class="row text-center">
<div class="col-12 col-md-12">{{ $package->count }}</div>                               
<div class="col-12 col-md-12">درس </div>                                 
</div>
</div>
</div>
</div>
...
<a style="padding-bottom : 5px" _target="blank" href="{{ Storage::disk('learniaa')->temporaryUrl( $current_course['download_link'], now()->addMinutes(120))}}" class="btn fourth btn-video btnblogPost">دانلود آموزش</a>                            
</div>
</div>
@else
<div class="col-md-12 text-center" >
<a href="{{ route('academy.course', ['pk_tree' => $tree->pk_tree ,'pk_package' =>  $package->pk_package ]) }}" class="btn fourth btn-round">خرید دوره {{$package['fa_name']}} </a>                           
</div>
</div>
@endif
<!-- Payment -->
...
<video class="afterglow" id="my-video" width="1920" height="1080" poster="{{Storage::url('course/'. $package['folder'] .'/' .$current_course['pic_cover'])}}" src="{{ Storage::disk('learniaa')->temporaryUrl( $current_course['download_link'], now()->addMinutes(120))}}"> </video>
</div>
</div>
</div>
<!-- VideoPlayer  -->
...
<img src="{{  Storage::url('learner/'.$current_course->learner['pic'])  }}" alt="{{$current_course->learner->user['name']}}" class="img-fluid rounded-circle shadow-lg" style="width: 90px;height:90px">                   
</div>
...
<div class="p-3 hover-style  container" style="margin-top:10px;border :2px solid #20c5ba">
<p style="text-align:justify">{{$current_course->learner['desc']}}</p>
</div>
</div>
</div>
</div>
